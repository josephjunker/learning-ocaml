...
let get_possibly_capturing_target board_state position =
  if not (position_exists position) then None else
    let occupying_piece = Map.find board_state.pieces_by_position position in
    match occupying_piece with
    | None -> Some (Move position)
    | Some { owner; _ } -> if compare_player board_state.turn owner = 0 then None else Some (Capture position)
...
let iterate_rider initial_position board_state offset =
  Sequence.unfold ~init:initial_position ~f:(fun current_position ->
    ...
    let target_move = get_possibly_capturing_target board_state new_position in
    match target_move with
    | Some move -> Some (move, new_position)
    | None -> None)
...
let knight_moves = fun board_state { row; column } ->
  List.filter_map [
    ...
    { row = row + 1; column = column - 2};
    { row = row - 1; column = column - 2};
  ] ~f:(fun new_position -> get_possibly_capturing_target board_state new_position)
...
let pawn_moves board_state initial_position =
  ...
  let direction = match current_player with
  | White -> 1
  | Black -> -1 in
  let { row; column } = initial_position in
  let not_yet_moved = match current_player with
  | White -> row = 2
  | Black -> row = 7 in
  ...
  | Some (_) -> None
  | None -> Some (Move forward_one_target) in
  ...
  | Some (_) -> None
  | None -> Some (Move forward_two_target) in
  ...
  | Some ({ owner; _}) -> if compare_player owner board_state.turn = 0 then None else Some (Capture capture_left_target)
  | _ -> None in
  ...
  | Some ({ owner = Black; _}) -> Some (Capture capture_right_target)
  | _ -> None in
  let en_passant_row = match current_player with
  | White -> 5
  | Black -> 4 in
  let en_passant = if not (row = en_passant_row) then None else match board_state.en_passant_valid_column with
  | Some col -> if (col - 1 = column) || (col + 1 = column)
      then Some (EnPassant { destination = { row = row + direction; column = col }; captured = { row; column = col }}) 
      else None
  | None -> None in
  List.filter_map [forward_one; forward_two; capture_left; capture_right; en_passant] ~f:(fun x -> x)
